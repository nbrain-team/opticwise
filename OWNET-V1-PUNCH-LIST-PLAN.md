...
- **Status (2026-05-13):** ✅ **SHIPPED & VERIFIED in production.** Commit `aaa737a` (cherry-picked clean from `sprint-1/3.5-deal-email-overmatch-fix` to avoid unrelated Insights/users work that had accumulated on that branch). Deployed live 2026-05-13 18:07 MDT on both `opticwise-frontend` and `opticwise-backend`. Render Gmail-sync cron (`crn-d6c9sv3h46gs738e4isg`) is healthy on `*/15 * * * *` schedule with fresh `lastSuccessfulRunAt` and `usersSynced=2`. Three production deals verified:
  - **Harlow / Aspen Oak** (0 stakeholders, no org) — `cmouc146f00i5oa2ah68a4i5k` → ~50 unrelated inbox emails → **1 legitimate email** (Kyle Clark, dealId-linked). Closes 3.1.
  - **PPP Starter Kit — Danny DeMichele** (1 stakeholder, `danny@nbrain.ai` + emailWork `ethan@nbrain.ai`) — `cmp1n8v1p000mph2aq3o02nqe` → ~95 emails, every one legitimately involves Danny or his nBrain colleagues. **Side-finding flagged for 3.6:** Danny's contact card has `emailWork = ethan@nbrain.ai` (data error — Ethan's email saved under Danny). Hygiene fix during 3.6 contact dedup.
  - **PPP Starter Kit — bill bobo** (1 stakeholder, `bill.douglas.co@gmail.com`) — `cmp34h5ov00a9pd29hgpnro7h` → 44 emails, every one genuinely contains `bill.douglas.co@gmail.com` in from/to/cc. This is a self-test deal where the stakeholder is Bill's own personal Gmail (forwards Bill sent to himself, contracts signed under that address); real customer deals won't behave this way.
- **Production-data observation:** Zero open deals currently have `Organization.domain` populated, so the inferred-tier UI (org-domain match with "Show inferred" toggle) is dormant until Bill starts filling org domains on real deals. Generic free-mail blocklist is in place from day one.
...
- **Questions for Bill:**
  ...
  4. For the "Draft email from transcript" feature, which standard outputs do you want as one-click buttons? (Follow-up, proposal outline, recap-to-team, LinkedIn DM, anything else?)
     - **Answer (2026-05-11):** **Category-aware buttons** — the button set shown on a transcript varies by meeting category. Proposed mapping (Bill to ratify in build review):
       - **Sales:** Follow-up email · Recap-to-team (Slack) · Proposal outline · LinkedIn DM · Discovery questions for next call · "Add to objection-handling canon"
       - **Client:** Follow-up email · Recap-to-team (Slack) · Action items as CRM tasks · "Add to wins library" · Renewal/expansion talking points
       - **Internal:** Action items as CRM tasks · Decision log entry · (no customer-facing or social outputs)
       - **Vendor:** Follow-up email · Vendor notes attached to the vendor record · Competitive intel summary (if applicable)
       - **Executives:** Board summary · Action items (private) · Decision log entry · (no Slack auto-post, no customer-facing outputs)
       - **PPP Podcast:** Show notes draft · Guest thank-you email · LinkedIn promo posts (Bill's voice + Drew's voice + neutral OW voice — three drafts) · Episode pull quotes for marketing · "Add to content backlog"
       - **Other:** Follow-up email (generic) · "Add to canon (manual review)"
     - **Build task implied:** (a) `TranscriptAction` config keyed by `MeetingCategory` with the button list per category; (b) generator framework that takes (transcript, action) and produces the right output using the right voice canon (Bill's voice for Bill-led sales calls, neutral for client check-ins led by other team members, Bill+Drew+neutral for podcast LinkedIn promos, etc.); (c) outputs land in: the user's email composer (per Section 4.4 Gap 2), the Deal/Contact notes, Slack channels (per 4.3 routing), or the content backlog canon doc — depending on action type; (d) all generated outputs are drafts only — never auto-send, always require human review.
- **Recommended approach (revised):** Build (in this order) — (i) `MeetingCategory` enum + auto-classifier; (ii) feedsTraining policy enforcement on canon writer; (iii) category-aware transcript action buttons; (iv) inline contact creator on transcript page (reuses Section 3.2's `<InlineCreatePicker>`); (v) Fathom → Read.ai cleanup pass.
- **Effort:** `M`–`L` (classifier is small; the per-category generator framework is most of the work; cleanup is `S`).
...
```
  ...
  - **New downstream considerations surfaced by the diagram:**
    1. **GrooveHQ** is the support ticketing system today. **Bill's plan (2026-05-11): migrate OFF GrooveHQ entirely** — OWnet absorbs support tickets. BUT **not in this v1 project**. So for now: GrooveHQ stays operational and untouched by v1 work. The full T1-Support-Agent-replaces-GrooveHQ migration is a separate, post-v1 project. Captured under Section 4.12 with the explicit out-of-scope-for-v1 marker.
    2. **Zapier is NOT actually in the flow (Bill, 2026-05-11).** The diagram was outdated. The real architecture is **direct webhook from Willow into OWnet CRM** — no Zapier middle layer. The web-form-to-CRM path is also direct (already wired via the FormEmbed → CRM bridge from April/May 2026). Bill will update the image; the canonical answer for the punch list is: **direct webhook in both paths, no Zapier**. Implementation work: confirm the `/api/webhooks/willow-postcall` (or equivalent) handler exists and is consuming Willow's payload natively; if not yet wired, that's the build task.
    ...
    4. **Press-1 (Support) — directional plan (Bill, 2026-05-11):** Once the T1 Support Agent ships (post-v1, separate project), **T1 fully replaces the live-agent queue**. AI answers first; escalates to human only if needed. Recorded now so the T1 Agent design (Section 4.12) is scoped against this target — full first-line replacement, not a fallback or add-on. The cutover happens after T1 has proven its quality bar.
...
- **Questions for Bill:**
  ...
  2. What channels must T1 cover? (Email to a support address, live chat on a portal, SMS, voice?)
     - **Answer (2026-05-11):** **All four customer-facing channels:**
       - **A — Email** (`support@opticwise.com`) — T1 reads, drafts in neutral OW brand voice, sends. Today's flow routes through GrooveHQ; T1 eventually replaces that per the post-v1 GrooveHQ migration ratified in Q49b.
       - **B — Live chat widget on a customer portal** — implies a customer portal exists (or one needs to be built as part of T1 scope). **Open dependency:** does an OW customer portal exist today? If yes, where, and is it embeddable? If no, the T1 project includes building/launching the portal. Capture as build-phase question.
       - **C — SMS** — requires inbound SMS via Twilio. Currently NOT in v1 scope (per Section 4.9 — Bill picked A+B only, no C). Since T1 is post-v1, inbound SMS will need to be added when T1 ships — adds a TCR campaign and inbound SMS handler to Twilio config. No conflict with v1 scope; just sequenced.
       - **D — Voice** — Customer dials the OITVOiP main line → presses 1 for Support → T1 handles via Willow-like voice agent (uses ElevenLabs voice ID `mzQCmfsyI83WHJxXgieu`, same as Willow per Q45). Per Q50 (ratified directional plan), this fully replaces the current live-agent call queue when T1 is mature.
...
- **Questions for Bill:**
  1. What docs do you need? Options: (a) system architecture diagram for OWnet for your reference / future contractors, (b) runbook for ops (what to do when X breaks), (c) data model reference (what tables exist, what they mean), (d) all of the above?
     - **Answer (2026-05-11):** **All four.** Staged build:
       - **(A) System Architecture Diagram** — `docs/architecture.md` with embedded diagram(s). Shows services (OWnet CRM, OWnet Agent, Content Engine, Forms, LinkedIn Manager, Knowledge Base, Slack Bot, Support Surface, Willow Voice, Read.ai ingester, Drive ingester, etc.), data stores (Postgres on Render, Pinecone vector store, S3 file storage), external integrations (Gmail, Slack, Read.ai, Twilio, ElevenLabs, OITVOiP, LinkedIn, X, Instagram, Drive, GrooveHQ for now), and trust boundaries (RBAC matrix from Section 4.2 overlay).
       - **(B) Ops Runbook** — `docs/runbook.md`. Practical "when X breaks, do Y" reference. Sections per failure mode: Render deploy fails, Gmail sync stalls, Pinecone times out, Slack bot stops responding, Read.ai webhook unhealthy, Twilio account suspended, ElevenLabs voice quota hit, LinkedIn rate-limited, customer email bounces, OWnet agent returns wrong answer + how to file a correction, etc. Each entry: detection, diagnosis steps, fix steps, escalation contact.
       - **(C) Data Model Reference** — `docs/data-model.md` autogenerated from `prisma/schema.prisma` plus human-written "why this exists" annotations per table/column. Includes relationships diagram, RBAC scope per table, retention policy per table.
       - **(D) Feature Catalog + User Docs** — `docs/features/` directory, one file per feature surface (CRM Deals, Contacts, Pipelines, Forms Admin, OWnet Agent, Content Engine, Knowledge Base, LinkedIn Manager, Social Composer, Support Surface, etc.). Per file: what it does, who can use it (RBAC tag), where the UI lives, common gotchas, related features. This file structure also doubles as the training corpus for Section 7 — OWnet ingests its own feature docs so the agent can answer "how do I do X in OWnet?".
...
- **Questions for Bill:**
  ...
  2. Format — recorded Loom walkthroughs (rewatchable), live sessions, written quick-starts, all three?
     - **Answer (2026-05-11):** **Three formats, all written or in-app — no video.**
       - **(A) Written quick-starts** — one-pager per surface ("Quick Start: Creating Your First Deal", "Quick Start: Setting Up a Form", etc.). 2–5 minute read each. Stored in the feature-catalog `docs/features/` directory from Section 6 so they live next to the engineering docs.
       - **(C) Step-by-step illustrated guides** — written walkthroughs with screenshots showing exactly where to click for each operation. Slower to produce but most self-explanatory for non-technical users. Stored alongside quick-starts; the illustrated guide is the "deep" version of a quick-start.
       - **(E) In-app tooltips / guided tour** — "Welcome — let me show you around" walkthrough triggered the first time a user logs into OWnet. Highest polish. Per-role variant: Executives see the Executive tour (CRM + Production future module + everything); Sales & Marketing see the sales tour; Ops & Engineering see the engineering tour; ASPR Onsite see the on-site tour. Each tour is 5–8 steps highlighting the surfaces relevant to that role.
