...
- **Recommended approach:** Build `/knowledge-base/train` UI on top of existing `KnowledgeDocument` infra: upload/URL fetch, corrections write to a separate `KnowledgeCorrection` table that gets retrieved with a 2× boost, every agent response gets a "View sources" expandable showing the chunks used.
- **Effort:** `L` (this is its own mini-feature)
