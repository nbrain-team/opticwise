/**
 * Sync Gmail messages with full pagination support
 * Vectorize and store in PostgreSQL
 * 
 * Usage:
 *   npx tsx scripts/sync-gmail.ts              # Default: 12 months
 *   npx tsx scripts/sync-gmail.ts --months=24  # Custom: 24 months
 *   npx tsx scripts/sync-gmail.ts --all        # All emails (no date filter)
 */
  ...
  try {
    ...
    for (const message of newMessages) {
      ...
      try {
        ...
        // Save to database using raw SQL to handle vector type
