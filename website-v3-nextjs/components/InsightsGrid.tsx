  ...
  const filtered = useMemo(() => {
    ...
    if (activeTag) {
      result = result.filter((p) =>
        p.tags?.some((t) => t.name === activeTag)
      );
    }
    if (search.trim()) {
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
            <div className="flex flex-wrap justify-center gap-2">
              <button
                ...
                className={`role-tab ${!activeTag ? "active" : ""}`}
              >
                All
              </button>
              {tags.map((tag) => (
                <button
                  ...
                  className={`role-tab ${activeTag === tag ? "active" : ""}`}
                >
                  {tag}
                </button>
              ))}
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-full text-sm focus:outline-none focus:border-ow-blue focus:ring-1 focus:ring-ow-blue/20"
              />
            </div>
          </div>
        </div>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-gray-400 text-lg">
                {posts.length === 0
                  ? "Blog posts will appear here once Ghost CMS is connected."
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {filtered.map((post) => (
                <Link
                  ...
                  className="group block bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-xl hover:-translate-y-1 hover:border-ow-blue/20 transition-all no-underline"
                >
