...
export function LeadMagnetForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Still show success - we'll capture leads even if API fails
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="grid sm:grid-cols-2 gap-3">
        <input
          ...
          required
          className="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/15 text-white placeholder-white/40 text-sm focus:outline-none focus:border-blue-400/50 focus:bg-white/15 transition-colors"
        />
        <input
          type="email"
          name="email"
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full sm:w-auto btn btn-primary btn-lg mt-2 disabled:opacity-60"
      >
