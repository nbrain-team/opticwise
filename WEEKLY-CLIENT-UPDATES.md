...
- Analyzed the entire **OpticWise Content Engine** project folder — Bill's canonical brand/voice/training corpus — and used it to ship a five-track upgrade to the OWnet agent so every response and every piece of generated content now sounds like the May 2026 brand, not the older snapshot the agent was running on
- **Track 1 — Canon ingested into the knowledge base.** New script `ow/scripts/ingest-content-engine-canon.ts` walks every `.md` / `.txt` / `.gs` / `.docx` in the Content Engine folder, classifies each file (Master SB7 BrandScript, Asset Manager Mindset Overlay, Sales Playbook, PPP Book manuscript, Bill's voice canon, Drew's voice canon, Voice Exemplars, Drive Bridge, Content Backlog), and loads them into `KnowledgeDocument` + `KnowledgeChunk` tagged `category = "Canon — *"` so retrieval boosts canon over generic Drive content. Idempotent on re-run; supports `--dry-run` and `--reingest`
- **Track 2 — BrandScript prompt rewritten to the May 2026 canon.** `ow/lib/brandscript-prompt.ts` now carries: the four-moats framework (data, workflows, orchestration, operating standard), the Stanford 2026 AI Index thesis ("the model is the commodity, the moat is the layer above it"), the Salwasser asset-manager-as-default-audience lens, separate **Bill** and **Drew** voice canons selectable via a new `author` parameter, the canonical signoff, the proof gate (no invented client metrics), the banned-words list (ESG, leverage, synergy, ecosystem, holistic, seamless, cutting-edge, PropTech-as-OpticWise, etc.), the trademark first-use rule (Property Brain™, Portfolio Brain™, PPP 5C™, BoT®, ElasticISP®, 5S®, SIC®, PPP Audit™, Peak Property Performance®), the production-ready spoken-voice analogies ("you own the car but it's 3,000 miles away", "right butt, wrong seat", "hand specialist"), and an opt-in **Content Engine Mode** that loads the eight-week moat rotation and the Roxy CMS load checklist
- **Voice enforcement upgraded to match.** `ow/lib/brandscript-voice-enforcement.ts` now promotes bare "infrastructure" → "data & digital infrastructure" (the canonical pair), strips ESG and the rest of the banned words contextually, and auto-applies trademark symbols on first use of every protected term. New exported `scoreCanonAdherence()` function returns a 0-100 canon-adherence score with itemized failures — used by the eval harness and surfaced in the Content Engine UI
- **Track 3 — Voice exemplars now retrieved at generation time.** New script `ow/scripts/ingest-voice-exemplars.ts` parses every published `.docx` blog package (Bill + Drew, April 19 and May 6 weeks, including supplementals) into Blog / LinkedIn Article / LinkedIn Short Post deliverables, embeds each, and stores them in `StyleGuide` with `category = "voice_exemplar"`, `author = "Bill" | "Drew"`, and `subcategory` matching the deliverable type. Added `getVoiceExemplars()` and `formatVoiceExemplars()` helpers in `lib/ai-agent-utils.ts` that pull top-K stylistically aligned past pieces and inject them as few-shot examples in the system prompt — wired into the chat route so any "draft a blog post / LinkedIn article / short-form post" intent automatically pulls Bill or Drew's matching exemplars
